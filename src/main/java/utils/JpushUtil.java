...
/**
 * @Author ZhangGJ
 * @Date 2020/06/08 17:54
 */
@Slf4j
public class JpushUtil {

    @Value("${jpush.app-key}")
    private static String appkey = "95c7850584c4af17e80a5c64";

    @Value("${jpush.master-secret}")
    private static String masterSecret = "3c3270a9c5f2ab4a76fd19f0";

    @Value("${jpush.live-time}")
    private static String liveTime = "1000";
     ...
     * @return PushResult
     */
    public static PushResult sendCustomPush(String title, String content, Map<String, String> extrasMap,
            String... alias) throws APIConnectionException, APIRequestException {
        ClientConfig clientConfig = ClientConfig.getInstance();
        clientConfig.setTimeToLive(Long.parseLong(liveTime));
     ...
     */
    public static PushResult sendPush(String title, String content, Map<String, String> extrasMap,
            String... tags) throws APIConnectionException, APIRequestException {
        ClientConfig clientConfig = ClientConfig.getInstance();
     ...
     */
    public static void sendPushWithCallback(String title, String content, Map<String, String> extrasMap,
            String... alias) {
        ClientConfig clientConfig = ClientConfig.getInstance();
        clientConfig.setTimeToLive(Long.valueOf(liveTime));
        String host = (String) clientConfig.get(ClientConfig.PUSH_HOST_NAME);
        NettyHttpClient client =
                new NettyHttpClient(ServiceHelper.getBasicAuthorization(appkey, masterSecret), null,
                        clientConfig);
        try {
     ...
     */
    public static void UpdateDeviceTagAlias(String registrationId, String alias, Set<String> tagsToAdd,
     ...
     * @return
     */
    public static PushPayload buildPushLoadByTags(String title, String content, Map<String, String> map,
            String... tags) {
        if (map.isEmpty()) {
            map = new HashMap<>();
        }
     ...
     * @return PushPayload
     */
    private static PushPayload buildPushPayload(String title, String content,
            Map<String, String> extrasMap, String... alias) {
        ...
        return PushPayload.newBuilder().setPlatform(Platform.android_ios())
                ...
                .setAudience((null == newAlias || newAlias.length == 0) ?
                        Audience.all() :
                ...
                .setNotification(Notification.newBuilder().setAlert(content)
                        .addPlatformNotification(AndroidNotification.newBuilder().setTitle(title)
                                .addExtras(extrasMap).build()).addPlatformNotification(
                                IosNotification.newBuilder().incrBadge(1).addExtras(extrasMap)
                                        .build()).build()).build();
    }
     ...
     * @return PushPayload
     */
    private static PushPayload buildCustomPushPayload(String title, String content,
            Map<String, String> extrasMap, String... alias) {
        // 批量删除数组中空元素
        String[] newAlias = removeArrayEmptyElement(alias);
        return PushPayload.newBuilder().setPlatform(Platform.android_ios()).setAudience(
                (null == newAlias || newAlias.length == 0) ? Audience.all() : Audience.alias(alias))
                .setNotification(Notification.newBuilder().setAlert(content)
                        .addPlatformNotification(AndroidNotification.newBuilder().setTitle(title)
                                .addExtras(extrasMap).build()).build()).setMessage(
                        Message.newBuilder().setTitle(title).setMsgContent(content)
                                .addExtras(extrasMap).build()).build();
    }
     ...
     * @return String[]
     */
    private static String[] removeArrayEmptyElement(String... strArray) {
